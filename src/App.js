...
function ClienteForm({ onVolver }) {
  ...
  function next() {
    ...
    if (idx+1 >= qs.length) {
      ...
      const sc = {}, cn = {};
      qs.forEach(({id,d}) => {
        const a = newAns[id]; if(!d||!a||typeof a.v!=="number") return;
  ...
  if (ph==="w") return (
    <div style={S}>
      ...
      <div style={{borderLeft:"3px solid #1a56db",paddingLeft:"1rem",marginBottom:"2rem"}}>
        ...
        <p style={{fontSize:13,color:"#888",margin:"4px 0 0"}}>Confidencial · Resultados para uso interno de Austral</p>
      </div>
    ...
    <div style={S}>
      ...
      <div style={{background:"#f9f9f8",borderRadius:12,padding:"1.25rem",marginBottom:"1.5rem"}}>
        ...
        {Object.entries(dims).sort((a,b)=>a[1]-b[1]).map(([d,s]) => {
          const meta=DM[d]||{n:d,c:"#888"}; const lvi=lvl(s);
          return (
      ...
      <div style={{textAlign:"center",padding:"2rem 1rem",background:"#f9f9f8",borderRadius:12,border:"1px solid #e5e7eb"}}>
        <p style={{fontSize:20,fontWeight:500,margin:"0 0 10px",color:"#212536"}}>Gracias por completar el diagnóstico.</p>
    ...
    <div style={S}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:"1.5rem"}}>
        ...
        <div style={{gridColumn:"span 2"}}>
          <label style={{display:"block",fontSize:12,color:"#666",marginBottom:5}}>Sector</label>
          <select value={inf.sec||""} onChange={e=>setInf(x=>({...x,sec:e.target.value}))} style={{width:"100%",boxSizing:"border-box",padding:"9px 12px",fontSize:13,borderRadius:8,border:"1px solid #e5e7eb",outline:"none"}}>
            <option value="">Seleccionar</option>
            {["Comercio y distribución","Servicios profesionales","Industria y manufactura","Construcción","Tecnología","Otro"].map(s=><option key={s} value={s}>{s}</option>)}
          </select>
        </div>
      </div>
    ...
    return (
      <div style={S}>
        <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4,marginBottom:"1rem"}}>
          {DA.map((d,dii)=>{
            const ds=dsc.find(x=>x.id===d.id); const act=dii===di;
            return (
    ...
    return (
      <div style={S}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:"1.5rem"}}>
          {sorted.map(ds=>{
            const meta=dm[ds.id]; const lvis=lvl(ds.s);
            return (
              <div key={ds.id} style={{background:"#f9f9f8",borderRadius:10,padding:"12px 14px",border:"1px solid #e5e7eb"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
                  {DA.find(d=>d.id===ds.id).ss.map((s,sii)=>{
                    const sv=sc[s.id]; const lvsub=lvl(sv!==undefined?sv:null);
                    return (
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <button onClick={()=>setVista("cliente")} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1.25rem 1.5rem",borderRadius:12,cursor:"pointer",textAlign:"left",border:"1.5px solid #1a56db33",background:"#fff",color:"#111"}}>
            <div>
              ...
              <p style={{margin:0,fontSize:13,opacity:.8}}>Diagnóstico inicial · Confidencial</p>
            </div>
